...
mod0 <- mgcv::gam(Nb_adults ~ Genetic_Diversity + Block +   s(Generation_Eggs, k=3) +
                    #s(Generation_Eggs, by = Genetic_Diversity, k=3) + 
                    s(Population, bs="re") + s(obs, bs = "re"), 
                    method="ML",  family = "poisson", link = log, data=data_dyn)
...
mod_2 <- mgcv::gam(Nb_adults ~ Genetic_Diversity + Block  +  s(Generation_Eggs, k=5) +
                    s(Population, bs="re"), data=data_dyn, family = "poisson", link = log, method='ML')
...
# The summary now provides the significance of the difference terms: Medium and High does show a significantly different trend over Time (F=3752, edf=3.78, p=0.01), Low and High does show significantly different trend over Time (F=5691, edf=3.92, p=0.01). 
...
mod_final <- mgcv::gam(Nb_adults ~ Genetic_Diversity + Block + 
                    ...
                    s(Population, bs="re"),  family = "poisson", link = log, data=data_dyn)
...
# mod_A_p$fit <- exp(mod_A_p$fit)
# mod_A_p$se.fit <- exp(mod_A_p$se.fit)
ggplot(data = mod_A_p, aes(Generation_Eggs, fit)) +
   tidymv::geom_smooth_ci(Genetic_Diversity) 


########## Other function EXCLUDING BLOCK EFFECT 
...
```{r }
########### 
########### OPTION E: COEFFICIENT OF VARIATION 
########### 
...
# cowplot::save_plot(here::here("figures","CoefficientVariation_time.pdf"),   plot_CV_function,
