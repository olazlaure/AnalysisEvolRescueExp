...
m1 <- gam(Nb_adults ~ s(Generation_Eggs), data=data_dyn)
...
m1 <- gam(Nb_adults ~ s(Generation_Eggs, k=3), data=data_dyn)
...
m.factor <- gam(Nb_adults ~ Genetic_Diversity +  s(Generation_Eggs, k=3) + 
...
m.factor <- gam(Nb_adults ~ Genetic_Diversity + Block +  s(Generation_Eggs, k=3) + 
            s(Generation_Eggs, by= Genetic_Diversity, k=3), data=data_dyn)
summary(m.factor)
...
m.mixed <- gam(Nb_adults ~ Genetic_Diversity + Block + s(Generation_Eggs, k=3) + 
...
# plot_smooth(m.mixed, view="Time", cond=list(Group="Adults"), 
#       rm.ranef=TRUE, main="intercept+s(Time):GroupAdults", rug=FALSE)
# 
# # Plot summed effects for Subject "a01":
# plot_smooth(m.mixed, view="Time", cond=list(Group="Adults", Subject="a01"), 
#       main="... + s(Subject)", col='red',
#       ylim=c(-15,20), rug=FALSE)
# # Add Subject "a18":
# plot_smooth(m.mixed, view="Time", cond=list(Group="Adults", Subject="a18"), 
#       add=TRUE, col='blue')
...
fit2 <- gam(Nb_adults ~ s(Generation_Eggs, by= Genetic_Diversity, k=3), data=data_dyn)
fit5 <- gam(Nb_adults ~ Genetic_Diversity + Block +  s(Generation_Eggs, k=5) + 
            s(Generation_Eggs, by= Genetic_Diversity, k=5) + 
              s(Population, bs="re"), data=data_dyn)

#library("visreg")
visreg::visreg(fit2, xvar = "Generation_Eggs",
       by = "Genetic_Diversity",
       data = data_dyn,
       method = "REML") 
...
########### AIC
########### 
fit1 <- gam(Nb_adults ~ s(Generation_Eggs, by= Genetic_Diversity, k=5), data=data_dyn)
fit2 <- gam(Nb_adults ~ s(Generation_Eggs, by= Genetic_Diversity, k=3), data=data_dyn)
fit3 <- gam(Nb_adults ~ Genetic_Diversity + Block +  s(Generation_Eggs, k=3) + 
            s(Generation_Eggs, by= Genetic_Diversity, k=3) + 
              s(Population, bs="re"), data=data_dyn)
fit4 <- gam(Nb_adults ~ Genetic_Diversity + Block +  s(Generation_Eggs, k=4) + 
...
fit <- multinom(stage ~ Divsersity + Week + 
                  ID_Rep + Population, data=data_multinom, weights=count)
summary(fit$fitted.values)
fit

fit0 <- multinom(stage ~ Divsersity + 
                   ID_Rep + Population , data=data_multinom, weights=count)
anova(fit,fit0)

fit1 <- multinom(stage ~ Week + 
                   ID_Rep + Population, data=data_multinom, weights=count)
anova(fit,fit1)
...
# #With random effect
# library("mclogit")
# mclogit::mclogit(cbind(prop,interaction(stage,ID_Rep)) ~ Week*Divsersity, data=data_multinom)
# 
# 
# 
# 
...
# 
# filldata <- data.frame(Divsersity = rep(levels(data_multinom$Divsersity),times=2), 
#                        Week = rep(levels(data_multinom$Week),each=3),
#                        Adults = NA, 
#                        Larvae = NA, 
#                        Pupae = NA)
# 
# 
# head(data_phenotyping_all)
