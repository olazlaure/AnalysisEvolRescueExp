---
title: "Beatrice Experiment - LO analysis"
...
# LIBRARIES
```{r }
#install.packages("name_of_Pacakge")
...
#library(corrplot) #fct corrplot()
# library(rr2) #Rsquare
# library(lsmeans) 
# library(blmeco) #overdispersion
# library(dplyr) #layout: function mutate()
# library(lemon) # multiplot: function g_legend()
...
```{r }
data <- read.table("DATA/BE_Census_Data.csv", sep=",", header=T)
...
#Order levels 
data$Genetic_Diversity <- revalue(data$Genetic_Diversity, c("Med"="Medium"))
...
data_he <- read.table("DATA/He_ReMaining_BeatricePop.csv", sep=",", header=T)
...
```{r }
#Upload growth rate end of experiment
data_phenotyping <- read.table("DATA/Adaptation_Data.csv", sep=",", header=T)
...
data_phenotyping$Genetic_Diversity <-   revalue(data_phenotyping$Genetic_Diversity, c("Med"="Medium"))
...
## Population size
```{r }
PLOT_Pop_size<-ggplot(data, aes(x = factor(Generation_Eggs), 
                                y = Nb_adults, 
                                group = Population,
  ...
  theme_LO_sobre
PLOT_Pop_size
# save_plot("PLOT_PopulationSize.pdf",   PLOT_Pop_size, base_height = 10/cm(1),
#           base_width = 15/cm(1), dpi = 610)



SUM_Popsize <- summarySE(data, 
...
# save_plot("PLOT_PopulationSize_average.pdf",   PLOT_Pop_size_average, base_height = 10/cm(1),
#           base_width = 15/cm(1), dpi = 610)
...
```{r }
## Extinction yes no
PLOT_Extinction<-ggplot(data, aes(x = factor(Generation_Eggs), 
                                y = Nb_adults, 
                                group = Population,
...
PLOT_Extinction
# #
# save_plot("PLOT_Extinction.pdf",   PLOT_Extinction, base_height = 6/cm(1),
...
PLOT_Growth<-ggplot(data_G2, aes(x = Genetic_Diversity, y = Lambda, 
                         colour = Genetic_Diversity)) + 
...
PLOT_Growth
# 
# save_plot("PLOT_Growth_Start.pdf",   PLOT_Growth, base_height = 10/cm(1),
...
SUM_MEAN_start_end <- summarySE(data_evolution_Lambda, measurevar = c("Lambda"),
                       groupvars = c("Genetic_Diversity","Phenotyping"), 
                       na.rm=TRUE)
SUM_POP_start_end <- summarySE(data_evolution_Lambda, measurevar = c("Lambda"),
                       groupvars = c("Genetic_Diversity","Phenotyping",
                                     "Population"), 
                       na.rm=TRUE)
...
# 
# save_plot("PLOT_Lambda_Evolution.pdf",
...
```{r }
SUM_Prop_adults<- summarySE(data_phenotyping_all, 
                        measurevar = c("p_adults"),
                       groupvars = c("Genetic_Diversity","Week"), 
                       na.rm = TRUE)
SUM_Prop_adults

SUM_Prop_pupae<- summarySE(data_phenotyping_all, 
                        measurevar = c("p_pupae"),
                       groupvars = c("Genetic_Diversity","Week"), 
                       na.rm = TRUE)
SUM_Prop_pupae

SUM_Prop_larvae<- summarySE(data_phenotyping_all, 
...
SUM_Prop<-gather(SUM_Prop,"Stage", "Proportion",4:6,-"Genetic_Diversity")
SUM_Prop
SUM_Prop$Stage <- factor(SUM_Prop$Stage, levels = c("p_adults", "p_pupae", "p_larvae"))


PLOT_prop <- ggplot(data = SUM_Prop, aes(x = Genetic_Diversity, y = Proportion, fill = Stage)) +
...
# save_plot("PLOT_Life_stage_proportion.pdf",   PLOT_prop,
...
ggplot(data_evolution_Lambda, aes(x = He_remain, y = Lambda, 
                         colour = Genetic_Diversity)) + 
...
PLOT_He_Initial <- ggplot(data_G2, aes(x = He_remain, y = Lambda, 
                         colour = Genetic_Diversity)) + 
  geom_point(size = 2, alpha = 0.8) +
  ylab("Growth rate") +
  theme(legend.position = "none") +
...
SUM_POP_He_Mean<- summarySE(data_evolution_Lambda[data_evolution_Lambda$Phenotyping=="Final",],
                            measurevar = c("Lambda"),
                       groupvars = c("Genetic_Diversity","He_remain_end",
                                     "Population"), 
                       na.rm=TRUE)

PLOT_He_Final <- ggplot(SUM_POP_He_Mean, aes(x = He_remain_end, y = Lambda, 
                         colour = Genetic_Diversity)) + 
  geom_point(size = 2, alpha = 0.8) +
  ylab("Growth rate") +
  theme(legend.position = "none") +
...
# ALL WITHOUT PSEUDO
SUM_POP_He_ALL<- summarySE(data_evolution_Lambda,
                            measurevar = c("Lambda"),
...
PLOT_He_ALL <- ggplot(SUM_POP_He_ALL, aes(x = HE, y = Lambda, 
                         colour = Genetic_Diversity, 
                         shape = Phenotyping)) + 
...
# PLOT_He_ALL <- ggplot(SUM_POP_He_ALL[SUM_POP_He_ALL$Phenotyping=="Final",], aes(x = HE, y = Lambda,
#                          colour = Genetic_Diversity,
#                          shape = Phenotyping)) +
#   geom_point(size = 3, alpha = 0.8) +
...
############ 
#Analysis
mod0 <- glmer(y ~ Genetic_Diversity + Block + (1|Population),  
...
mod1 <- glmer(y ~ Block + (1|Population),  
      data = data_proba_extinction, family = binomial)
anova(mod0, mod1, test = "Chisq")
#We keep genetic diversity 


mod0 <- glmer(y ~ Genetic_Diversity-1 + Block + (1|Population),  
      data = data_proba_extinction, family = binomial)
summary(mod0)
...
############ 
#Analysis
# mod0 <- glmer(Nb_adults ~ Genetic_Diversity + Block + (1|Population),  
...
#second degree
fit2 <- glm(Nb_adults~poly(Generation_Eggs,2,raw=TRUE), data = data, family = "poisson")
#third degree
fit3 <- glm(Nb_adults~poly(Generation_Eggs,3,raw=TRUE), data = data, family = "poisson")
#fourth degree
fit4 <- glm(Nb_adults~poly(Generation_Eggs,4,raw=TRUE), data = data, family = "poisson")
#fourth degree
fit5 <- glm(Nb_adults~poly(Generation_Eggs,5,raw=TRUE), data = data, family = "poisson")
#exp
...
#third degree
fit3 <- lm(log(Nb_adults+1)~poly(Generation_Eggs,3,raw=TRUE), data = data[data$Generation_Eggs>=2,])
#fourth degree
fit4 <- lm(log(Nb_adults+1)~poly(Generation_Eggs,4,raw=TRUE), data = data[data$Generation_Eggs>=2,])
#fourth degree
fit5 <- lm(log(Nb_adults+1)~poly(Generation_Eggs,5,raw=TRUE), data = data[data$Generation_Eggs>=2,])
#exp
...
fit4 <- glmer(Nb_adults~poly(Generation_Eggs,4,raw=TRUE)*Genetic_Diversity + (1|Block), data = data, family = "poisson")
blmeco::dispersion_glmer(fit4)

mod <- glmer(Nb_adults~poly(Generation_Eggs,4,raw=TRUE)*Genetic_Diversity + 
...
#Plot
PLOT_Pop_size <- ggplot(data = data) + 
...
mod <- glmer(Nb_adults~poly(Generation_Eggs,4,raw=TRUE)*Genetic_Diversity + 
               (1|Block) + (1|obs), data = data, family = "poisson")
mod1 <- glmer(Nb_adults~poly(Generation_Eggs,4,raw=TRUE)+Genetic_Diversity + 
               (1|Block) + (1|obs), data = data, family = "poisson")
...
fit_fin <- glm(Nb_adults~poly(Generation_Eggs,4,raw=TRUE)*Genetic_Diversity,
...
# Square root
msqrt <- lmer(sqrt(Lambda) ~ Genetic_Diversity*Phenotyping  + 
                Block + (1|Population), 
              data = data_evolution_Lambda)
summary(msqrt)

# log
mlog <- lmer(log(Lambda+1) ~ Genetic_Diversity*Phenotyping  +
               Block + (1|Population),
              data=data_evolution_Lambda)

# Normal
mnorm <- lmer(Lambda ~ Genetic_Diversity*Phenotyping  + 
                Block + (1|Population),
              data=data_evolution_Lambda)
...
#Test normality
g1 <- gofstat(fitdist(residuals(mlog), "norm"))
g1$kstest 
g1 <- gofstat(fitdist(residuals(msqrt), "norm"))
g1$kstest 
g1 <- gofstat(fitdist(residuals(mnorm), "norm"))
g1$kstest
...
m0 <- lmer(Lambda ~ Genetic_Diversity*Phenotyping  + 
                Block + (1|Population) ,
              data=data_evolution_Lambda)
...
m2 <- lmer(Lambda ~ Phenotyping  + 
                Block + (1|Population),
              data=data_evolution_Lambda)
m3 <- lmer(Lambda ~ Genetic_Diversity  + 
                Block + (1|Population),
              data=data_evolution_Lambda)
...
###### Posthoc
############ 
mod_final <- lmer(Lambda ~ Genetic_Diversity+Phenotyping  + 
                Block + (1|Population),
              data=data_evolution_Lambda)
...
## Fitness G6 
```{r }
m0 <- glmer(Adults ~ Genetic_Diversity  + (1|Population) + (1|ID_Rep:Population),
                family = "poisson", data = data_phenotyping)
summary(m0)

# Equivalent to:
# m0 <- glmer(Adults ~ Genetic_Diversity  + (1|Population/ID_Rep),
#                 family = "poisson", data = data_5week)


#dispersion_glmer(m0) #dispersion ok 
#Note: (1|School) + (1|Class:School) is equivalent to (1|School/Class)

m0 <- glmer(Adults ~ Genetic_Diversity  + (1|Population) + (1|ID_Rep:Population),
                family = "poisson", data = data_phenotyping)
m1 <- glmer(Adults ~ 1  + (1|Population) + (1|ID_Rep:Population),
                family = "poisson", data = data_phenotyping)
...
#test double nested
m0 <- glmer(Adults ~ Genetic_Diversity  + (1|Genetic_Diversity/Population/ID_Rep),
                family = "poisson", data = data_phenotyping)
m1 <- glmer(Adults ~ 1  + (1|Genetic_Diversity/Population/ID_Rep),
                family = "poisson", data = data_phenotyping)
anova(m0,m1,test="Chisq") 

m0 <- glmer(Adults ~ Genetic_Diversity  + (1|Genetic_Diversity/ID_Rep),
                family = "poisson", data = data_phenotyping)
m1 <- glmer(Adults ~ 1  + (1|Genetic_Diversity/ID_Rep),
                family = "poisson", data = data_phenotyping)
anova(m0,m1,test="Chisq") 
...
#Calcul for end:
summarySE(data_G2[data_G2$Extinction_finalstatus!="yes",],
