...
#### Deal with the no event in diverse issue
```{r }
...
#########
######### Difference in the model between med and low

#Extraction of the difference 
mod_1 <- logistf::logistf(y ~ Genetic_Diversity-1 + Block, 
                      data=data_proba_extinction)
summary(mod_1)
obs_diff <- plogis(coef(mod_1)[3]) - plogis(coef(mod_1)[2])
#plogis(-2.3929983)-plogis(-2.5593086)
names(obs_diff) <- "Observed difference"
obs_diff      

# Parametric bootstrap interval for the difference
reps <- 5000
diff_boot <- sapply(1:reps, simul_modlogist)
mean(diff_boot)


hist(diff_boot)
abline(v = obs_diff, col = "red")
quantile(diff_boot, c(0.025, 0.975))  #95% interval
obs_diff
...
#Extraction of the difference 
mod_1 <- glm(Generation_Eggs ~ Genetic_Diversity-1,
                    data = data_timeextinction, family = "poisson")
summary(mod_1)
obs_diff <- exp(coef(mod_1)[2]) - exp(coef(mod_1)[1])
names(obs_diff) <- "Observed difference"
obs_diff      
...
# Parametric bootstrap interval for the difference
reps <- 5000
diff_boot <- rep(NA, reps)
for ( i in 1:reps ) {
    dat_i <- simulate(mod_1)
    dat_i <- data.frame(Genetic_Diversity = data_timeextinction$Genetic_Diversity, 
                        y = dat_i$sim_1)
    mod1_i <- glm(y ~ Genetic_Diversity - 1, data = dat_i, family = "poisson")
    diff_boot[i] <- exp(coef(mod1_i)[2]) - exp(coef(mod1_i)[1])
}
hist(diff_boot)
abline(v = obs_diff, col = "red")
quantile(diff_boot, c(0.025, 0.975))  #95% interval
...
plot_GAM_Ucurve_interaction <- ggplot(data = data, aes(Generation_Eggs, Nb_adults)) +
 ...
 # 
# cowplot::save_plot(file = here::here("figures","Popsize_Twitt.pdf"),
#                    plot_GAM_Ucurve_interaction, base_height = 10/cm(1),
...
cowplot::save_plot(file = here::here("figures","Popsize_CNRS.pdf"),
plot_GAM_Ucurve_interaction, base_height = 4,
base_width = 6, dpi = 610)

```


## Variance over time: Coefficient of variation 
```{r }
...
#Extraction of the difference 
mod_1 <- lme4::lmer(Lambda ~ Genetic_Diversity-1 + Block + (1|Population), 
                   data=data_phenotyping)
sum_mod_1 <- summary(mod_1)
obs_diff <- sum_mod_1$coefficients[2,1] - sum_mod_1$coefficients[3,1]
names(obs_diff) <- "Observed difference"
obs_diff      
...
for ( i in 1:reps ) {
    dat_i <- simulate(mod_1)
    dat_i <- data.frame(Genetic_Diversity = data_phenotyping$Genetic_Diversity,
                        Block = data_phenotyping$Block, 
                        Population = data_phenotyping$Population, 
                        Lambda_expected = dat_i$sim_1)
    mod1_i <- lme4::lmer(Lambda_expected ~ Genetic_Diversity-1 + Block + (1|Population), 
                   data=dat_i)
    sum_mod_1 <- summary(mod1_i)
    diff_boot[i] <- sum_mod_1$coefficients[2,1] - sum_mod_1$coefficients[3,1]
}
hist(diff_boot)
abline(v = obs_diff, col = "red")
quantile(diff_boot, c(0.025, 0.975))  #95% interval
...
mod3 <- ordinal::clmm(stage ~ Week-1 + Block +
                        (1|ID_Rep) + (1|Population), 
                      data=data_multinom, weights=count)
mod3$coefficients
confint(mod3)
