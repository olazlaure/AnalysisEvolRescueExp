#' Function to simulate y in logistf() linear model 
#'
#' @description Function to simulate y in logistf() linear model 
#' @param seed Seed used for simulation
#' @param mod Give the structure of the model
...
simul_modlogist <- function(seed = 1){
  ...
  sim_data$simy_logit <- rbinom(n=nrow(sim_data),
                               size = 1, 
                               prob = (1/(1+exp(-(sim_data$FixEff))))) / 1
  sim_data$simy <- asin(sqrt(sim_data$simy_logit))
